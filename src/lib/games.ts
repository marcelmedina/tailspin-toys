...
function filterClauseFor(filters: GameFilters) {
    const conditions = [];
...
/** Return all games optionally narrowed by category and publisher filters. */
export async function getGames(db: Database, filters: GameFilters = {}): Promise<Game[]> {
    const query = baseGamesQuery(db);
    const clause = filterClauseFor(filters);
...
/** All categories ordered alphabetically for filter controls. */
...
/** All publishers ordered alphabetically for filter controls. */
